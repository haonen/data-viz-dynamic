...
function processData(data, year) {
    ...
    data.filter(d => {
      return d.year === year;
        ...
        obj.push([d.Location, [+d.CountRate, +d.Employer]])
    });
    var processedData = Object.assign(new Map(obj), {title: ["# Plans Per Captia", "Employer Coverage"]});
...
export default function Employer(data) {
    ...
    // i think there's a bug here, you are just getting a big array of arryay
            ...
            .map(([i, j]) => {
                ...
                return `<rect width=${k} height=${k} x=${i * k} y=${(n - 1 - j) * k} fill=${color}>
                        <title>#Plans Per Capita${labels[j] && ` (${labels[j]})`}Employer Coverage${labels[i] &&
                ` (${labels[i]})`}</title>
                </rect>`;
            })
            .join('\n');
    const legend = `<g class=legend-container font-family=sans-serif font-size=10 transform=translate(900,600)>
            ...
            2})">
                                ...
                                text-anchor="middle">Employer Coverage</text>
                            </g>
                            </g>`;
